...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    CGFloat angle = velocity.x == 0.0 ?: atanf(fabs(velocity.y / velocity.x));
...
- (void)scrollAnimationFrame:(CADisplayLink*)cdl
{
    ...
    CGFloat movementPerSecond = CLAMP(fabs(diff)*14, 20, 4000)*fsign(diff);
    CGFloat movement = movementPerSecond * delta;
    
    if (fabs(movement) > fabs(diff)) movement = diff; // so we never step over the target point
...
- (void)layoutSubviews
{
    ...
    for(SPStackedPageContainer *pageC in self.subviews) {
        ...
        if (actualPen.origin.x <= 0 && pageC != [self.subviews lastObject]) {
            pageC.overlayOpacity = 0.3/actualPen.size.width*fabs(actualPen.origin.x);
        } else {
...
- (void)updateContainerVisibilityByShowing:(BOOL)doShow byHiding:(BOOL)doHide
{
    BOOL bouncing = self.scrollAnimationTimer && fabs(_targetOffset.x - _actualOffset.x) < 30;
